...
#define tt2 2*tt
uint_32 s[tt2+1]; // Syndrome values

uint_32 rr;//redundant length		// BCH code parameters


uint_32 p[mm + 1];
uint_32 alpha_to[nn+1], index_of[nn+1] ;	// Galois field
uint_32 gg[rr_max+1] ;		// Generator polynomial
...
uint_32 ggx4=0;
// get crc32 value
uint_32 CRC_32(unsigned char* pData, uint_32 ulSize)
{
    uint_32 i;
    uint_32 nAccum = 0;
...
unsigned short CRC_16(unsigned char* aData, uint_32 aSize)
{
    uint_32 i;
...
void P_RC4(unsigned char* buf, unsigned int len)
{
...
unsigned short CRC_CCITT(unsigned char* p, uint_32 CalculateNumber)
{
...
void gen_poly()
{	
	uint_32 gen_roots[nn + 1], gen_roots_true[nn + 1] ; 	// Roots of generator polynomial
	uint_32 i, j, Temp ;
