...
NOTES:			Provides an interface to the CMD5Checksum class. 'strFilePath' name should 
				...
				exception.
*****************************************************************************************/
string CMD5Checksum::GetMD5(const string& strFilePath,long long nLength)
{
...
unsigned char * CMD5Checksum::_GetMD5(const string& strFilePath,long long nLength)
{
	//open the file as a binary file in readonly mode, denying write access 
	FILE *file=NULL;
	file = fopen(strFilePath.c_str(),"rb");
...
*****************************************************************************************/
string CMD5Checksum::GetMD5(FILE *File,long long nLength,long long nOffset)
{
	try
	{
		...
		uint_8 Buffer[nBufferSize];		//buffer for data read from the file
...
FUNCTION:		CMD5Checksum::GetMD5
DETAILS:		static, public
DESCRIPTION:	Gets the MD5 checksum for data in a uint_8 array
RETURNS:		CString : the hexadecimal MD5 checksum for the specified data
ARGUMENTS:		uint_8* pBuf  :	pointer to the uint_8 array
				uint_32 nLength :	number of BYTEs of data to be checksumed
NOTES:			Provides an interface to the CMD5Checksum class. Any data that can
				be cast to a uint_8 array of known length can be checksummed by this
				function. Typically, CString and char arrays will be checksumed, 
				although this function can be used to check the integrity of any uint_8 array. 
				A buffer of zero length can be checksummed; all buffers of zero length 
				will return the same checksum. 
*****************************************************************************************/
string CMD5Checksum::GetMD5(uint_8* pBuf, uint_32 nLength)
{
...
unsigned char *  CMD5Checksum::_GetMD5(uint_8* pBuf, uint_32 nLength)
{
	//calculate and return the checksum
	CMD5Checksum MD5Checksum;
	MD5Checksum.Update( pBuf, nLength );
...
DETAILS:		private
DESCRIPTION:	Rotates the bits in a 32 bit uint_32 left by a specified amount
RETURNS:		The rotated uint_32 
ARGUMENTS:		uint_32 x : the value to be rotated
				int n   : the number of bits to rotate by
*****************************************************************************************/
uint_32 CMD5Checksum::RotateLeft(uint_32 x, int n)
{
	//check that uint_32 is 4 bytes long - true in Visual C++ 6 and 32 bit Windows
	assert( sizeof(x) == 4 );
...
RETURNS:		none
ARGUMENTS:		uint_32 &A, B, C, D : Current (partial) checksum
				uint_32 X           : Input data
				uint_32 S			  : MD5_SXX Transformation constant
				uint_32 T			  :	MD5_TXX Transformation constant
NOTES:			None
*****************************************************************************************/
void CMD5Checksum::FF( uint_32& A, uint_32 B, uint_32 C, uint_32 D, uint_32 X, uint_32 S, uint_32 T)
{
	uint_32 F = (B & C) | (~B & D);
	A += F + X + T;
	A = RotateLeft(A, S);
...
NOTES:			None
*****************************************************************************************/
void CMD5Checksum::GG( uint_32& A, uint_32 B, uint_32 C, uint_32 D, uint_32 X, uint_32 S, uint_32 T)
{
	uint_32 G = (B & D) | (C & ~D);
	A += G + X + T;
	A = RotateLeft(A, S);
...
NOTES:			None
*****************************************************************************************/
void CMD5Checksum::HH( uint_32& A, uint_32 B, uint_32 C, uint_32 D, uint_32 X, uint_32 S, uint_32 T)
{
	uint_32 H = (B ^ C ^ D);
	A += H + X + T;
	A = RotateLeft(A, S);
...
NOTES:			None
*****************************************************************************************/
void CMD5Checksum::II( uint_32& A, uint_32 B, uint_32 C, uint_32 D, uint_32 X, uint_32 S, uint_32 T)
{
	uint_32 I = (C ^ (B | ~D));
	A += I + X + T;
	A = RotateLeft(A, S);
...
RETURNS:		void
ARGUMENTS:		uint_32* Output : the 32 bit (unsigned long) destination array 
				uint_8* Input	  : the 8 bit (unsigned char) source array
				uint_32 nLength  : the number of 8 bit data items in the source array
NOTES:			Four BYTES from the input array are transferred to each uint_32 entry
				of the output array. The first uint_8 is transferred to the bits (0-7) 
				of the output uint_32, the second uint_8 to bits 8-15 etc. 
...
*****************************************************************************************/
void CMD5Checksum::ByteToDWord(uint_32* Output, uint_8* Input, uint_32 nLength)
{

	//initialisations
	uint_32 i=0;	//index to Output array
	uint_32 j=0;	//index to Input array

	//transfer the data by shifting and copying
	for ( ; j < nLength; i++, j += 4)
	{
		Output[i] = (uint_32)Input[j]			| 
					(uint_32)Input[j+1] << 8	| 
					(uint_32)Input[j+2] << 16 | 
					(uint_32)Input[j+3] << 24;
	}
}
...
*****************************************************************************************/
void CMD5Checksum::Transform(uint_8 Block[64])
{
...
RETURNS:		void
ARGUMENTS:		uint_8* Output  : the 8 bit destination array 
				uint_32* Input  : the 32 bit source array
				uint_32 nLength  : the number of 8 bit data items in the source array
NOTES:			One uint_32 from the input array is transferred into four BYTES 
				in the output array. The first (0-7) bits of the first uint_32 are 
				transferred to the first output uint_8, bits bits 8-15 are transferred from
				the second uint_8 etc. 
...
*****************************************************************************************/
void CMD5Checksum::DWordToByte(uint_8* Output, uint_32* Input, uint_32 nLength )
{
	//transfer the data by shifting and copying
	uint_32 i = 0;
	uint_32 j = 0;
	for ( ; j < nLength; i++, j += 4) 
	{
		Output[j] =   (uint_8)(Input[i] & 0xff);
		Output[j+1] = (uint_8)((Input[i] >> 8) & 0xff);
		Output[j+2] = (uint_8)((Input[i] >> 16) & 0xff);
		Output[j+3] = (uint_8)((Input[i] >> 24) & 0xff);
	}
}
...
string CMD5Checksum::Final()
{
	...
	uint_32 nIndex = (uint_32)((m_nCount[0] >> 3) & 0x3f);
	uint_32 nPadLen = (nIndex < 56) ? (56 - nIndex) : (120 - nIndex);
...
RETURNS:		void
ARGUMENTS:		uint_8* Input    : input block
				uint_32 nInputLen : length of input block
NOTES:			Computes the partial MD5 checksum for 'nInputLen' bytes of data in 'Input'
*****************************************************************************************/
void CMD5Checksum::Update( uint_8* Input,	uint_32 nInputLen )
{
