...
public:
	...
	static string GetMD5(uint_8* pBuf, uint_32 nLength);
	static string GetMD5(FILE *File,long long nLength,long long nOffset=0);
	static string GetMD5(const string& strFilePath,long long nLength);
	static string GetMD5(const string& strFilePath);
	static unsigned char * _GetMD5(uint_8* pBuf, uint_32 nLength);
    static unsigned char * _GetMD5(FILE *File,long long nLength);
    static unsigned char * _GetMD5(const string& strFilePath,long long nLength);
protected:
	...
	void Transform(uint_8 Block[64]);
	void Update(uint_8* Input, uint_32 nInputLen);
	string Final();
	unsigned char * FinalChar();
	inline uint_32 RotateLeft(uint_32 x, int n);
	...
	//utility functions
	void DWordToByte(uint_8* Output, uint_32* Input, uint_32 nLength);
	void ByteToDWord(uint_32* Output, uint_8* Input, uint_32 nLength);

private:
	uint_8* m_pBuffer;
	uint_8  m_lpszBuffer[64];		//input buffer
	uint_32 m_nCount[2];			//number of bits, modulo 2^64 (lsb first)
	uint_32 m_lMD5[4];			//MD5 checksum
};
