...
void lock_process_mutex() {
  int iRet;
  if (global_mutex.ptr) {
    iRet = pthread_mutex_lock(global_mutex.ptr);
    if (iRet == EOWNERDEAD)//previous owner is dead
      pthread_mutex_consistent(global_mutex.ptr);
  }
}
void unlock_process_mutex() {
  if (global_mutex.ptr)
    pthread_mutex_unlock(global_mutex.ptr);
}